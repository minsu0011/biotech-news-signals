"""Frozen Alpaca SIP retry for annual-report-consensus DEV candidates."""
...
POLICY_PATH = DATA / "ALPACA_ANNUAL_CONSENSUS_RETRY_POLICY_V1.json"
ANNUAL_POLICY_PATH = DATA / "HISTORICAL_ANNUAL_REPORT_SYMBOL_CONSENSUS_POLICY_V1.json"
...
REPORT_PATH = RESEARCH / "ALPACA_ANNUAL_CONSENSUS_RETRY_REPORT.json"
...
def load_policy() -> tuple[dict[str, Any], str]:
    ...
    required = {
        "schema_version": 1,
        "policy_id": "ALPACA_SIP_ANNUAL_CONSENSUS_RETRY_V1",
    ...
    if any(policy.get(key) != value for key, value in required.items()):
        raise RuntimeError("ANNUAL_ALPACA_RETRY_POLICY_GUARD_FAIL")
    bindings = {
        resolver.QUEUE_PATH: policy["activation_queue_sha256"],
        DEV_PATH: policy["activation_dev_extension_sha256"],
        ANNUAL_AUDIT_PATH: policy["activation_annual_audit_sha256"],
        PARITY_PATH: policy["activation_parity_summary_sha256"],
        ANNUAL_POLICY_PATH: policy["required_annual_policy_sha256"],
    }
    for path, expected in bindings.items():
        if resolver.sha256(path) != expected:
            raise RuntimeError(f"ANNUAL_ALPACA_ACTIVATION_BINDING_MISMATCH:{path.name}")
    return policy, resolver.sha256(POLICY_PATH)


def select_candidates(queue: pd.DataFrame, policy: dict[str, Any]) -> pd.DataFrame:
    # Read identity only; y/returns/prices never enter candidate selection.
    ...
    mask &= queue.annual_consensus_status.astype(str).eq(
        str(policy["required_annual_consensus_status"])
    )
    mask &= queue.annual_consensus_contract_sha256.astype(str).eq(
        str(policy["required_annual_policy_sha256"])
    )
...
def main() -> int:
    ...
    candidate_ids = sorted(candidates.event_id.astype(str))
    digest = hashlib.sha256("\n".join(candidate_ids).encode()).hexdigest()
    if len(candidates) != int(policy["candidate_count"]) or digest != policy["candidate_event_ids_sha256"]:
        raise RuntimeError("ANNUAL_ALPACA_CANDIDATE_SET_MISMATCH")
    ...
    for (symbol, date), group in candidates.groupby(["resolved_ticker", "event_date"], sort=False):
        if attempted_groups >= int(policy["maximum_requests"]):
            break
        attempted_groups += 1
        ...
        for index in group.index:
            queue.loc[index, "annual_alpaca_retry_policy_sha256"] = policy_sha
            queue.loc[index, "annual_alpaca_retry_attempt_count"] = 1
