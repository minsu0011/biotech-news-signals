"""Frozen Alpaca SIP retry for exact-event-accession archive identities."""
...
POLICY_PATH = DATA / "ALPACA_EVENT_ACCESSION_ARCHIVE_RETRY_POLICY_V1.json"
IDENTITY_POLICY_PATH = DATA / "HISTORICAL_EVENT_ACCESSION_ARCHIVE_SYMBOL_POLICY_V1.json"
COMPLIANCE_POLICY_PATH = DATA / "EVENT_ARCHIVE_FULL_SUBMISSION_EXCLUSION_REPAIR_POLICY_V2.json"
COMPLIANCE_AUDIT_PATH = RESEARCH / "EVENT_ARCHIVE_FULL_SUBMISSION_EXCLUSION_REPAIR_V2_AUDIT.json"
PARITY_PATH = RESEARCH / "PROVIDER_PARITY_SUMMARY.json"
DEV_PATH = DATA / "V59_DEV_EXTENSION_LABELED.csv.gz"
REPORT_PATH = RESEARCH / "ALPACA_EVENT_ACCESSION_ARCHIVE_RETRY_REPORT.json"
...
def load_policy() -> tuple[dict[str, Any], str]:
    ...
    required = {
        "schema_version": 1,
        "policy_id": "ALPACA_SIP_EVENT_ACCESSION_ARCHIVE_RETRY_V1",
    ...
    if any(policy.get(key) != value for key, value in required.items()):
        raise RuntimeError("EVENT_ARCHIVE_ALPACA_RETRY_POLICY_GUARD_FAIL")
    bindings = {
        resolver.QUEUE_PATH: policy["activation_queue_sha256"],
        DEV_PATH: policy["activation_dev_extension_sha256"],
        COMPLIANCE_AUDIT_PATH: policy["activation_compliance_audit_sha256"],
        PARITY_PATH: policy["activation_parity_summary_sha256"],
        IDENTITY_POLICY_PATH: policy["required_identity_policy_sha256"],
        COMPLIANCE_POLICY_PATH: policy["required_compliance_repair_policy_sha256"],
    }
    for path, expected in bindings.items():
        if resolver.sha256(path) != expected:
            raise RuntimeError(f"EVENT_ARCHIVE_ALPACA_BINDING_MISMATCH:{path.name}")
...
def select_candidates(queue: pd.DataFrame, policy: dict[str, Any]) -> pd.DataFrame:
    ...
    mask &= queue.event_archive_exclusion_repair_status.astype(str).eq(
        str(policy["required_compliance_repair_status"])
    )
    mask &= queue.event_archive_exclusion_repair_policy_sha256.astype(str).eq(
        str(policy["required_compliance_repair_policy_sha256"])
    )
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("EVENT_ARCHIVE_ALPACA_CANDIDATE_SET_MISMATCH")
    ...
    for (symbol, date), group in candidates.groupby(
        ...
        for index in group.index:
            queue.loc[index, "event_archive_alpaca_retry_policy_sha256"] = policy_sha
            queue.loc[index, "event_archive_alpaca_retry_attempt_count"] = 1
