"""Frozen Alpaca SIP retry for annual-plus-prospectus DEV candidates."""
...
POLICY_PATH = DATA / "ALPACA_ANNUAL_PLUS_PROSPECTUS_RETRY_POLICY_V1.json"
IDENTITY_POLICY_PATH = DATA / "HISTORICAL_ANNUAL_PLUS_PROSPECTUS_SYMBOL_CONSENSUS_POLICY_V1.json"
IDENTITY_AUDIT_PATH = RESEARCH / "HISTORICAL_ANNUAL_PLUS_PROSPECTUS_SYMBOL_CONSENSUS_AUDIT.json"
PARITY_PATH = RESEARCH / "PROVIDER_PARITY_SUMMARY.json"
DEV_PATH = DATA / "V59_DEV_EXTENSION_LABELED.csv.gz"
REPORT_PATH = RESEARCH / "ALPACA_ANNUAL_PLUS_PROSPECTUS_RETRY_REPORT.json"
...
def load_policy() -> tuple[dict[str, Any], str]:
    ...
    required = {
        "schema_version": 1,
        "policy_id": "ALPACA_SIP_ANNUAL_PLUS_PROSPECTUS_RETRY_V1",
    ...
    if any(policy.get(key) != value for key, value in required.items()):
        raise RuntimeError("PROSPECTUS_ALPACA_RETRY_POLICY_GUARD_FAIL")
    bindings = {
    ...
    for path, expected in bindings.items():
        if resolver.sha256(path) != expected:
            raise RuntimeError(f"PROSPECTUS_ALPACA_ACTIVATION_BINDING_MISMATCH:{path.name}")
    return policy, resolver.sha256(POLICY_PATH)


def select_candidates(queue: pd.DataFrame, policy: dict[str, Any]) -> pd.DataFrame:
    # Only event identity membership is read; y/returns/prices are not loaded.
...
def main() -> int:
    ...
    candidate_ids = candidates.event_id.astype(str).tolist()
    digest = hybrid.event_ids_sha256(candidate_ids)
    if (
        len(candidates) != int(policy["candidate_count"])
        or digest != policy["candidate_event_ids_sha256"]
    ):
        raise RuntimeError("PROSPECTUS_ALPACA_CANDIDATE_SET_MISMATCH")
    ...
    for (symbol, date), group in candidates.groupby(
        ...
        for index in group.index:
            queue.loc[index, "annual_plus_prospectus_alpaca_retry_policy_sha256"] = policy_sha
            queue.loc[index, "annual_plus_prospectus_alpaca_retry_attempt_count"] = 1
