"""V126 multichannel Gramian-angular-field direction challenger.
...
horizons.  Each channel trajectory is deterministically clipped and mapped to
angles.  The exact Gramian angular summation field (GASF) upper triangle and
Gramian angular difference field (GADF) strict upper triangle yield a fixed
512-dimensional nonlinear time-image representation.  A train-only robust
feature scale and one fixed duplicate/class-balanced ridge-logistic head predict
direction.

There is no learned image encoder, threshold/radius selection, RQA line statistic,
wavelet, rough-path signature, SPD covariance, Koopman/DMD operator, DTW, HDC,
polynomial chaos, GMM/Fisher vector, rank objective, source/text/ticker identity,
or external data.  Inner-past evidence selects exact V69 or fixed .25/.50 blends;
outer labels remain evaluation-only under a strict 35-minute embargo.  V69
confidence/high-confidence remain exact and material failure restores the exact
entire V69 DataFrame.

Audit and US:2 smoke use CPU30-31/two threads/no GPU and write nothing.  Full
...
DEFAULT_OUT = ROOT / "research" / "local_run_outputs" / "V126_MULTICHANNEL_GAF_V1"
VERSION = 126
HYPOTHESIS = "MULTICHANNEL_GRAMIAN_ANGULAR_FIELD_DIRECTION_V1"
...
HORIZON_N = 8
STATE_CLIP = 3.0
GASF_TERMS_PER_CHANNEL = 36
GADF_TERMS_PER_CHANNEL = 28
GAF_FEATURE_DIMENSION = CHANNEL_N * (GASF_TERMS_PER_CHANNEL + GADF_TERMS_PER_CHANNEL)
FEATURE_CLIP = 8.0
...
SMOKE_THREADS = 2
ARCHITECTURES = (
    {"name": "MULTICHANNEL_GAF_W0.25", "weight": 0.25},
    {"name": "MULTICHANNEL_GAF_W0.50", "weight": 0.50},
)
...
def verify_static_spec() -> dict[str, Any]:
    require(len(scaffold.CHANNEL_SPEC) == CHANNEL_N and len(scaffold.HORIZON_GRID) == HORIZON_N, "V126 path geometry changed")
    require(GAF_FEATURE_DIMENSION == 512, "V126 GAF feature dimension changed")
    return {
        "channel_n": CHANNEL_N,
        "channel_names": list(scaffold.CHANNEL_NAMES),
        "horizon_n": HORIZON_N,
        "horizon_grid_minutes": scaffold.HORIZON_GRID.tolist(),
        "robust_state_clip": [-STATE_CLIP, STATE_CLIP],
        "angular_map": "phi=arccos(clip(robust_state,-3,3)/3)",
        "gasf": "cos(phi_i+phi_j), upper triangle including diagonal",
        "gadf": "sin(phi_i-phi_j), strict upper triangle",
        "gasf_terms_per_channel": GASF_TERMS_PER_CHANNEL,
        "gadf_terms_per_channel": GADF_TERMS_PER_CHANNEL,
        "feature_dimension": GAF_FEATURE_DIMENSION,
...
def observed_path(robust_numeric: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    path = joint[:, 1:, 1:]
    require(path.shape[1:] == (HORIZON_N, CHANNEL_N), "V126 observed path slice changed")
    return path, {
        "rows": len(path),
...
def gramian_angular_features(path: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    require(path.ndim == 3 and path.shape[1:] == (HORIZON_N, CHANNEL_N), "V126 GAF path shape invalid")
    cosine = np.clip(path, -STATE_CLIP, STATE_CLIP) / STATE_CLIP
    sine = np.sqrt(np.maximum(0.0, 1.0 - cosine * cosine))
    channel_cosine = np.transpose(cosine, (0, 2, 1))
    channel_sine = np.transpose(sine, (0, 2, 1))
    gasf = (
        channel_cosine[:, :, :, None] * channel_cosine[:, :, None, :]
        - channel_sine[:, :, :, None] * channel_sine[:, :, None, :]
    )
    gadf = (
        channel_sine[:, :, :, None] * channel_cosine[:, :, None, :]
        - channel_cosine[:, :, :, None] * channel_sine[:, :, None, :]
    )
    upper_diag = np.triu_indices(HORIZON_N, k=0)
    upper_strict = np.triu_indices(HORIZON_N, k=1)
    gasf_feature = gasf[:, :, upper_diag[0], upper_diag[1]]
    gadf_feature = gadf[:, :, upper_strict[0], upper_strict[1]]
    feature = np.concatenate([gasf_feature, gadf_feature], axis=2).reshape(len(path), -1)
    require(feature.shape[1] == GAF_FEATURE_DIMENSION, "V126 GAF feature count mismatch")
    require(np.isfinite(feature).all() and float(feature.min()) >= -1.0000000001 and float(feature.max()) <= 1.0000000001, "V126 GAF range invalid")
    gasf_symmetry = float(np.max(np.abs(gasf - np.swapaxes(gasf, 2, 3))))
    gadf_skew = float(np.max(np.abs(gadf + np.swapaxes(gadf, 2, 3))))
    require(gasf_symmetry <= 1e-12 and gadf_skew <= 1e-12, "V126 GAF algebraic symmetry failed")
    return feature, {
        "rows": len(feature),
        "feature_dimension": feature.shape[1],
        "feature_min": float(feature.min()),
        "feature_max": float(feature.max()),
        "gasf_symmetry_max_abs_error": gasf_symmetry,
        "gadf_skew_symmetry_max_abs_error": gadf_skew,
...
class RobustFeatureScale:
    ...
    def fit(self, matrix: np.ndarray) -> "RobustFeatureScale":
        require(matrix.ndim == 2 and matrix.shape[1] == GAF_FEATURE_DIMENSION, "V126 scale shape changed")
        ...
        require(np.isfinite(self.median).all() and np.isfinite(self.scale).all(), "V126 scale invalid")
        return self

    def transform(self, matrix: np.ndarray) -> np.ndarray:
        result = np.clip((matrix - self.median) / self.scale, -FEATURE_CLIP, FEATURE_CLIP)
        require(np.isfinite(result).all(), "V126 scaled GAF invalid")
        return result


def gaf_direction(train: pd.DataFrame, target_frame: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ordered = train.sort_values(["event_time_utc", "event_id"], kind="stable").reset_index(drop=True)
    require(ordered.market.nunique() == 1 and target_frame.market.nunique() == 1, "V126 expects one market per fit")
    ...
    train_feature, train_gaf_audit = gramian_angular_features(train_path)
    target_feature, target_gaf_audit = gramian_angular_features(target_path)
    ...
    require(np.isfinite(probability).all(), "V126 probability invalid")
    return probability, {
        "train_n": len(ordered),
        ...
        "train_gaf": train_gaf_audit,
        "target_gaf": target_gaf_audit,
        "gaf_scaling": {
            "train_only": True,
            "clip": FEATURE_CLIP,
        ...
        "target_rows_used_for_robust_scale_gaf_scale_or_head": False,
        "target_labels_used": False,
        "learned_image_encoder": False,
        "rqa_threshold_or_line_statistics": False,
        "haar_signature_spd_dmd_or_dtw": False,
        "hdc_or_polynomial_chaos": False,
        "gmm_fisher_vector": False,
        "rank_or_hodges_lehmann": False,
        "prediction": {
...
def choose_inner(
    ...
    challenger, model_audit = gaf_direction(inner_train, inner_valid)
    ...
    return {
        "selection_rule": "inner-past OOF only; exact V69 no-op or fixed .25/.50 Gramian-angular-field blend; fixed 2*AUC+BA with BA/net eligibility",
        "baseline": baseline_metric,
        "selected": selected,
        "trials": trials,
        "outer_labels_used_for_selection": False,
        "state_clip_field_terms_head_or_blend_micro_tuning": False,
    }, model_audit
...
def run_nested(
    ...
    diagnostic["v126_model"] = "V69_NOOP"
    ...
    for market, fold in (fold_specs[:1] if smoke else fold_specs):
        ...
        outer_chronology = chronology(outer_train, outer_valid, f"V126 outer {market} fold {fold}")
        ...
        challenger, outer_model_audit = gaf_direction(outer_train, outer_valid)
        ...
        diagnostic.loc[positions, "v126_model"] = selected["name"]
        outer_diagnostics = {
        ...
        evidence["gaf_prob"] = challenger
        evidence["candidate_prob"] = candidate
        evidence["v126_model"] = selected["name"]
        ...
        print(
            f"[V126 GAF] market={market} fold={fold} selected={selected['name']} "
            f"inner_auc_delta={selected['auc_delta']:+.6f} outer_auc_delta={audits[-1]['outer_auc_delta']:+.6f}",
            flush=True,
        )
    evidence = pd.concat(evidence_parts, ignore_index=True)
    require(evidence.event_id.is_unique, "V126 evidence repeats an event")
    require(np.array_equal(champion.confidence_signal.to_numpy(float), diagnostic.confidence_signal.to_numpy(float)), "V126 changed confidence")
    require(np.array_equal(champion.high_conf.to_numpy(bool), diagnostic.high_conf.to_numpy(bool)), "V126 changed high_conf")
...
def paired_nested_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str, Any]:
    ...
    require(len(blocks) >= 12, "too few V126 nested-bootstrap blocks")
    ...
    require(len(values["auc_delta"]) >= int(0.90 * draws), "V126 bootstrap lost too many draws")
    ...
    return {
        ...
        "method": "paired market-fold-time block bootstrap over inner-locked V126 Gramian-angular-field policy",
        "seed": SEED,
...
def evaluate(
    ...
    require(candidate_summary["research_gate"] == canonical_candidate, "V126 canonical mismatch")
    ...
    gaf_contract = all(
        ...
        and audit[side]["static_spec"]["feature_dimension"] == GAF_FEATURE_DIMENSION
        and not audit[side]["static_spec"]["artificial_origin_or_time_coordinate_used"]
        and audit[side]["train_gaf"]["feature_dimension"] == GAF_FEATURE_DIMENSION
        and audit[side]["train_gaf"]["label_independent_exact_transform"]
        and audit[side]["target_gaf"]["gasf_symmetry_max_abs_error"] <= 1e-12
        and audit[side]["target_gaf"]["gadf_skew_symmetry_max_abs_error"] <= 1e-12
        and audit[side]["gaf_scaling"]["train_only"]
        and not audit[side]["target_rows_used_for_robust_scale_gaf_scale_or_head"]
        and not audit[side]["target_labels_used"]
        and not audit[side]["learned_image_encoder"]
        and not audit[side]["rqa_threshold_or_line_statistics"]
        and not audit[side]["haar_signature_spd_dmd_or_dtw"]
        and not audit[side]["hdc_or_polynomial_chaos"]
        and not audit[side]["gmm_fisher_vector"]
        and not audit[side]["rank_or_hodges_lehmann"]
    ...
    checks = {
        ...
        "multichannel_gramian_angular_field_contract_verified": gaf_contract,
    }
    ...
    if not material_pass:
        require(selected_frame.equals(champion), "V126 fallback is not exact entire V69")
    return {
...
def material_gate(evaluation: dict[str, Any]) -> dict[str, Any]:
    ...
    require(gate["nested_bootstrap"]["contract_key"] == "selected.material_gate.nested_bootstrap", "V126 nested key mismatch")
    return gate
...
def write_outputs(
    ...
    require(bool(CONTROLLER_OUTPUT), "V126 full output requires MARKET_BIO_VERSION_OUTPUT authority")
    require(out.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V126 output is not controller-bound")
    ...
    reports = {
        "MODEL_COMPARISON.json": {
            "version": "V126", "hypothesis": HYPOTHESIS, "status": evaluation["status"],
            "models": {
                "V69_CHAMPION": evaluation["baseline_summary"],
                "V126_DIAGNOSTIC_MULTICHANNEL_GAF": evaluation["candidate_summary"],
                "V126_FAIL_CLOSED_SELECTED": selected,
            },
        ...
        "V126_MULTICHANNEL_GRAMIAN_ANGULAR_FIELD_REPORT.json": {
            "version": "V126", "hypothesis": HYPOTHESIS, "static_spec": STATIC_SPEC,
            "architectures": ARCHITECTURES, "nested_fold_audits": audits,
            "evaluation": compact, "authority_audit": authority,
        },
        "CANONICAL_RESEARCH_GATE_AUDIT.json": {
            "version": "V126", "status": "MATCH", "canonical_check_count": 14,
    ...
    atomic_csv(evidence, out / "V126_MULTICHANNEL_GAF_OUTER_EVIDENCE.csv.gz")
    atomic_csv(evaluation["selected_frame"], out / "V126_FAIL_CLOSED_SELECTED_OOF.csv.gz")
...
def parse_args() -> argparse.Namespace:
    ...
    if CONTROLLER_OUTPUT:
        require(
            ...
            "explicit mode/output conflicts with controller-bound no-argument V126 full run",
        )
        args.full_run = True
    ...
    if args.full_run:
        require(bool(CONTROLLER_OUTPUT), "V126 direct full run forbidden; MARKET_BIO_VERSION_OUTPUT required")
...
def main() -> None:
    ...
    if args.audit_only:
        print(json.dumps(clean({
            ...
            "architecture": "fixed exact multichannel GASF/GADF time-image plus one balanced ridge-logistic head",
            ...
            "controller_contract": {
                ...
                "research_staging_v126_compatible": True,
