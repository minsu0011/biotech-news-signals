"""V78 date/source-relative pairwise return-ranking challenger on GPU.

This is a ranking objective, not binary classification, MultiClass return
classification, continuous/quantile regression, confidence correctness, or an
economic selector.  Inside each training UTC-date x source-family group, the
target is only the relative ordering of realized 30-minute returns.  A
CatBoostRanker with PairLogit learns those within-group preferences.  Raw
target scores are mapped through the past training-score empirical CDF, never
through the current target batch, then blended into committed V69 with one of
two fixed weights or exact no-op selected on inner-past evidence.

Authority is immutable V36 DEV plus atomic output_V69 only.  Folds 2--4 use
expanding nested chronology, a strict 35-minute embargo, event/duplicate
purge, inner-only coarse selection, and outer evaluation-only outcomes.  V69
confidence and high-confidence membership are immutable.  Material failure
returns the exact entire V69 frame.  Audit and one-fold GPU smoke are allowed
before a separate explicit full controller run.
"""
...
DEFAULT_OUT = ROOT / "research" / "staging" / "V78_DATE_SOURCE_PAIRWISE_RETURN_RANK_GPU_V1"

VERSION = 78
HYPOTHESIS = "DATE_SOURCE_PAIRWISE_SIGNED_RETURN_RANK_GPU_V1"
...
FULL_ITERATIONS = 420
SMOKE_ITERATIONS = 80
BLEND_WEIGHTS = (0.20, 0.40)
BOOTSTRAP_DRAWS = 2000

CATEGORICAL_COLUMNS = (
    "market", "source_family", "source", "form", "event_type", "ticker",
    "market_event", "source_event", "family_form", "session_bucket",
)
NUMERIC_COLUMNS = (
...
def probability_blend(baseline: np.ndarray, rank_score: np.ndarray, weight: float) -> np.ndarray:
    base_logit = logit(np.clip(np.asarray(baseline, float), 1e-5, 1.0 - 1e-5))
    rank_logit = logit(np.clip(np.asarray(rank_score, float), 1e-5, 1.0 - 1e-5))
    return expit((1.0 - float(weight)) * base_logit + float(weight) * rank_logit)
...
def prepare_dev(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    output["rank_group_key"] = (
        output.event_time_utc.dt.strftime("%Y-%m-%d") + "|" + output.source_family.fillna("UNKNOWN").astype(str)
    )
    ...
    output["session_bucket"] = pd.cut(
        minutes, [-np.inf, 30, 120, 270, np.inf], labels=["OPEN", "MORNING", "MIDDAY", "CLOSE"]
    ).astype(str)
...
def comparable_pairs(frame: pd.DataFrame) -> int:
    total = 0
    for _, group in frame.groupby("rank_group_key", sort=False):
        counts = group.fwd_ret_30m.value_counts().to_numpy(int)
        n = len(group)
        total += int((n * (n - 1) - np.sum(counts * (counts - 1))) // 2)
    return total


def ranking_training_frame(train: pd.DataFrame) -> pd.DataFrame:
    stats = train.groupby("rank_group_key").fwd_ret_30m.agg(["size", "nunique"])
    eligible_keys = set(stats.index[(stats["size"] >= 2) & (stats["nunique"] >= 2)])
    ranked = train.loc[train.rank_group_key.isin(eligible_keys)].copy()
    ranked = ranked.sort_values(["rank_group_key", "event_time_utc", "event_id"], kind="stable").reset_index(drop=True)
    ranked["relative_return_relevance"] = ranked.groupby("rank_group_key").fwd_ret_30m.rank(
        method="average", pct=True
    )
    require(len(ranked) >= 500, "insufficient pairwise ranking rows")
    require(comparable_pairs(ranked) >= 500, "insufficient comparable return pairs")
    return ranked


def empirical_cdf(reference: np.ndarray, target: np.ndarray) -> np.ndarray:
    reference = np.sort(np.asarray(reference, float))
    target = np.asarray(target, float)
    require(np.isfinite(reference).all() and np.isfinite(target).all(), "non-finite ranking score")
    # Past training-score CDF only; current target composition is never used.
    positions = np.searchsorted(reference, target, side="right")
    return np.clip((positions + 0.5) / (len(reference) + 1.0), 1e-5, 1.0 - 1e-5)


def fit_gpu_ranker(
    train: pd.DataFrame, target: pd.DataFrame, seed: int, smoke: bool
) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    ranked = ranking_training_frame(train)
    group_id = pd.factorize(ranked.rank_group_key, sort=False)[0].astype(np.int64)
    pool = Pool(
        feature_frame(ranked), label=ranked.relative_return_relevance.to_numpy(float),
        group_id=group_id, cat_features=list(CATEGORICAL_COLUMNS),
    )
    ...
    model.fit(pool)
    require(str(model.get_param("task_type")).upper() == "GPU", "ranker did not use GPU")
    train_raw = np.asarray(model.predict(pool), float)
    target_raw = np.asarray(model.predict(target_pool), float)
    rank_probability = empirical_cdf(train_raw, target_raw)
    return rank_probability, target_raw, {
        "library": "catboost", "catboost_version": catboost.__version__,
        "model": "CatBoostRanker", "loss_function": "PairLogit",
        "task_type": model.get_param("task_type"), "devices": model.get_param("devices"),
        "actual_gpu_fit_completed": True,
        "iterations": model.get_param("iterations"), "depth": model.get_param("depth"),
        "training_rows": len(ranked), "training_groups": int(ranked.rank_group_key.nunique()),
        "comparable_pairs": comparable_pairs(ranked),
        "target_rows": len(target),
        "target_batch_used_for_score_normalization": False,
        "score_normalization": "empirical CDF of fitted past training raw scores",
    }
...
def inner_partition(
    ...
    audit = chronology(inner_train, inner_valid, "V78 inner")
...
def choose_inner(
    ...
    rank_score, raw_score, model_audit = fit_gpu_ranker(train, valid, SEED + fold * 10, smoke)
    trials = [{
    ...
    for weight in BLEND_WEIGHTS:
        probability = probability_blend(baseline, rank_score, weight)
        ...
        trials.append({
            "name": f"PAIRWISE_RANK_W{weight:.2f}", "weight": float(weight),
    ...
    selected = max(
        ...
        key=lambda trial: (trial["score"], trial["auc_delta"], trial["net_delta"], -trial["weight"]),
    )
    return {
        "selection_rule": "maximize 2*AUC+BA+10*net on inner-past OOF; BA delta>=-0.005, net delta>=-0.0005; no-op and fixed 0.20/0.40 blends only",
        "ranking_group": "UTC date x source_family in training only",
        "ranking_target": "within-group percentile order of fwd_ret_30m",
        "selected": selected, "trials": trials,
        "gpu_model_audit": model_audit,
        "inner_raw_rank_score_sha256": array_sha256(raw_score),
...
def run_nested(
    ...
    diagnostic["v78_model"] = "V69_NOOP"
    diagnostic["v78_rank_weight"] = 0.0
    evidence_parts = []
    audits = []
    for fold in (2, 3, 4):
        ...
        outer_audit = chronology(outer_train, outer_valid, f"V78 outer fold {fold}")
        ...
        rank_score, raw_score, outer_model_audit = fit_gpu_ranker(
        ...
        candidate = baseline.copy() if weight == 0.0 else probability_blend(baseline, rank_score, weight)
        ...
        diagnostic.loc[positions, "v78_model"] = selected["name"]
        diagnostic.loc[positions, "v78_rank_weight"] = weight
        ...
        evidence["rank_probability"] = rank_score
        evidence["raw_rank_score"] = raw_score
        evidence["candidate_prob"] = candidate
        evidence["v78_model"] = selected["name"]
        evidence["v78_rank_weight"] = weight
        evidence_parts.append(evidence)
        audits.append({
            "fold": fold, "inner_chronology": inner_audit, "outer_chronology": outer_audit,
            "policy": policy, "outer_gpu_model_audit": outer_model_audit,
        ...
        print(
            f"[V78 GPU RANK] fold={fold} selected={selected['name']} "
            f"inner_auc_delta={selected['auc_delta']:+.6f} "
            f"outer_auc_delta={audits[-1]['outer_auc_delta']:+.6f}", flush=True,
        )
        if smoke:
            break
...
def paired_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str, Any]:
    ...
    return {
        "method": "paired fold/date block bootstrap over locked ranking policies",
...
def evaluate(
    ...
    require(baseline_summary["research_gate"] == canonical_base, "baseline canonical gate mismatch")
    require(candidate_summary["research_gate"] == canonical_candidate, "candidate canonical gate mismatch")
    ...
    checks = {
        ...
        "outer_net_delta_gt_0": candidate["all_trade_mean_signed_net"] - base["all_trade_mean_signed_net"] > 0.0,
        ...
        "all_pairwise_gpu_fits_completed": all(
            audit["actual_gpu_fit_completed"] and audit["task_type"] == "GPU"
            and audit["loss_function"] == "PairLogit" for audit in gpu_audits
        ),
        "target_batch_not_used_for_rank_normalization": all(
            not audit["target_batch_used_for_score_normalization"] for audit in gpu_audits
        ),
    ...
    if not material_pass:
        require(selected_frame.equals(champion), "fallback is not exact V69")
    return {
        ...
        "gpu": {"fit_count": len(gpu_audits), "all_actual_pairwise_gpu": checks["all_pairwise_gpu_fits_completed"]},
        "direction_change": {
...
def write_outputs(
    ...
    require(not out.name.startswith("output_V78"), "runner must not write output_V78 directly")
    ...
    reports = {
        "MODEL_COMPARISON.json": {
            "version": "V78", "hypothesis": HYPOTHESIS, "status": evaluation["status"],
            "models": {
                "V69_CHAMPION": evaluation["baseline_summary"],
                "V78_PAIRWISE_RANK_DIAGNOSTIC": evaluation["candidate_summary"],
                "V78_FAIL_CLOSED_SELECTED": evaluation["selected_summary"],
            },
        ...
        "V78_PAIRWISE_RETURN_RANK_REPORT.json": {
            "version": "V78", "hypothesis": HYPOTHESIS,
            "objective": "PairLogit within UTC-date x source-family return-order groups",
            "blend_weights": BLEND_WEIGHTS,
            "target_batch_normalization": False,
            "authority_audit": authority, "nested_fold_audits": audits, "evaluation": compact,
        },
        "V78_IMMUTABILITY_AUDIT.json": {
    ...
    atomic_csv(evidence, out / "V78_PAIRWISE_RETURN_RANK_OUTER_OOF.csv.gz")
    atomic_csv(evaluation["selected_frame"], out / "V78_FAIL_CLOSED_SELECTED_OOF.csv.gz")
    return {"runner_files_written": sorted([*reports, "V78_PAIRWISE_RETURN_RANK_OUTER_OOF.csv.gz", "V78_FAIL_CLOSED_SELECTED_OOF.csv.gz"])}
...
def main() -> None:
    ...
    if args.audit_only:
        print(json.dumps(clean({
            ...
            "model_contract": {
                "class": "CatBoostRanker", "loss": "PairLogit",
                "task_type": "GPU", "device_ordinal": 0,
                "ranking_group": "UTC date x source_family",
            },
    ...
    summary = {
        ...
        "gpu_pairwise_fit_count": evaluation["gpu"]["fit_count"],
        "all_actual_pairwise_gpu": evaluation["gpu"]["all_actual_pairwise_gpu"],
