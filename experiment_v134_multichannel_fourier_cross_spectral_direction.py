"""V134 multichannel causal Fourier cross-spectral direction challenger.

Every embargoed same-market train cut robust-scales only the immutable 37 causal
numeric fields and maps them to eight semantic channels on eight pre-event
horizons.  After removing each channel's row-wise DC level, an exact normalized
real DFT supplies four nonzero-frequency log auto-powers and deterministic
pairwise unit cross-spectrum real/imaginary phase couplings.  This fixed
256-dimensional frequency-domain representation is train-only robust-scaled,
then one fixed duplicate/class-balanced ridge-logistic head predicts direction.

There is no random Fourier/RBF approximation, learned frequency or threshold,
DMD transition operator, echo-state reservoir, Gramian angular image, wavelet,
rough-path signature, recurrence plot, source/text/ticker identity, calibration,
or post-outer tuning. Inner-past evidence selects exact V69 or fixed .25/.50
blends; outer labels remain evaluation-only under a strict 35-minute embargo.
...
DEFAULT_OUT = ROOT / "research" / "local_run_outputs" / "V134_MULTICHANNEL_FOURIER_CROSS_SPECTRAL_V1"
VERSION = 134
HYPOTHESIS = "MULTICHANNEL_CAUSAL_FOURIER_CROSS_SPECTRAL_DIRECTION_V1"
...
CHANNEL_N = 8
HORIZON_N = 8
NONZERO_FREQUENCY_N = HORIZON_N // 2
CHANNEL_PAIR_N = CHANNEL_N * (CHANNEL_N - 1) // 2
AUTO_POWER_DIMENSION = CHANNEL_N * NONZERO_FREQUENCY_N
CROSS_PHASE_DIMENSION = CHANNEL_PAIR_N * NONZERO_FREQUENCY_N * 2
SPECTRAL_FEATURE_DIMENSION = AUTO_POWER_DIMENSION + CROSS_PHASE_DIMENSION
...
SMOKE_THREADS = 2
ARCHITECTURES = (
    {"name": "FOURIER_CROSS_SPECTRAL_W0.25", "weight": 0.25},
    {"name": "FOURIER_CROSS_SPECTRAL_W0.50", "weight": 0.50},
)
...
def verify_static_spec() -> dict[str, Any]:
    require(len(scaffold.CHANNEL_SPEC) == CHANNEL_N and len(scaffold.HORIZON_GRID) == HORIZON_N, "V134 path geometry changed")
    require(NONZERO_FREQUENCY_N == 4 and CHANNEL_PAIR_N == 28, "V134 frequency/pair count changed")
    require(SPECTRAL_FEATURE_DIMENSION == 256, "V134 spectral feature dimension changed")
    return {
        "channel_n": CHANNEL_N,
        "channel_names": list(scaffold.CHANNEL_NAMES),
        "horizon_n": HORIZON_N,
        "horizon_grid_minutes": scaffold.HORIZON_GRID.tolist(),
        "dc_removed_per_row_channel": True,
        "transform": "exact numpy rfft over causal horizon axis, orthonormal normalization",
        "nonzero_frequency_indices": [1, 2, 3, 4],
        "auto_power": "log1p squared complex magnitude",
        "cross_phase": "real and imaginary parts of unit normalized complex cross-spectrum",
        "channel_pair_n": CHANNEL_PAIR_N,
        "auto_power_dimension": AUTO_POWER_DIMENSION,
        "cross_phase_dimension": CROSS_PHASE_DIMENSION,
        "feature_dimension": SPECTRAL_FEATURE_DIMENSION,
        "learned_frequency_threshold_or_band": False,
...
def observed_path(robust_numeric: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    path = joint[:, 1:, 1:]
    require(path.shape[1:] == (HORIZON_N, CHANNEL_N), "V134 observed path slice changed")
    return path, {
        "rows": len(path),
...
def fourier_cross_spectral_features(path: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    require(path.ndim == 3 and path.shape[1:] == (HORIZON_N, CHANNEL_N), "V134 spectral path shape invalid")
    channel_path = np.transpose(path, (0, 2, 1))
    centered = channel_path - np.mean(channel_path, axis=2, keepdims=True)
    spectrum = np.fft.rfft(centered, axis=2, norm="ortho")[:, :, 1:]
    require(spectrum.shape[1:] == (CHANNEL_N, NONZERO_FREQUENCY_N), "V134 rfft shape changed")
    squared_magnitude = np.abs(spectrum) ** 2
    auto_power = np.log1p(squared_magnitude).reshape(len(path), -1)
    left, right = np.triu_indices(CHANNEL_N, k=1)
    cross = spectrum[:, left, :] * np.conjugate(spectrum[:, right, :])
    denominator = np.sqrt(
        squared_magnitude[:, left, :] * squared_magnitude[:, right, :] + SPECTRAL_EPSILON
    )
    unit_cross = cross / denominator
    cross_phase = np.concatenate([unit_cross.real, unit_cross.imag], axis=2).reshape(len(path), -1)
    feature = np.concatenate([auto_power, cross_phase], axis=1)
    one_sided_weights = np.asarray([2.0, 2.0, 2.0, 1.0], dtype=float)
    spectral_energy = np.sum(squared_magnitude * one_sided_weights[None, None, :], axis=2)
    time_energy = np.sum(centered * centered, axis=2)
    parseval_error = float(np.max(np.abs(spectral_energy - time_energy)))
    require(feature.shape[1] == SPECTRAL_FEATURE_DIMENSION, "V134 spectral feature count mismatch")
    require(np.isfinite(feature).all() and parseval_error <= 1e-10, "V134 spectral algebra failed")
    require(float(np.max(np.abs(unit_cross))) <= 1.0000000001, "V134 unit cross-spectrum bound failed")
    return feature, {
        "rows": len(feature),
        "feature_dimension": feature.shape[1],
        "auto_power_dimension": auto_power.shape[1],
        "cross_phase_dimension": cross_phase.shape[1],
        "parseval_max_abs_error": parseval_error,
        "unit_cross_max_abs": float(np.max(np.abs(unit_cross))),
...
class RobustFeatureScale:
    ...
    def fit(self, matrix: np.ndarray) -> "RobustFeatureScale":
        require(matrix.ndim == 2 and matrix.shape[1] == SPECTRAL_FEATURE_DIMENSION, "V134 scale shape changed")
        ...
        require(np.isfinite(self.median).all() and np.isfinite(self.scale).all(), "V134 scale invalid")
        return self

    def transform(self, matrix: np.ndarray) -> np.ndarray:
        result = np.clip((matrix - self.median) / self.scale, -FEATURE_CLIP, FEATURE_CLIP)
        require(np.isfinite(result).all(), "V134 scaled spectral features invalid")
        return result


def spectral_direction(train: pd.DataFrame, target_frame: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ordered = train.sort_values(["event_time_utc", "event_id"], kind="stable").reset_index(drop=True)
    require(ordered.market.nunique() == 1 and target_frame.market.nunique() == 1, "V134 expects one market per fit")
    ...
    train_feature, train_spectral_audit = fourier_cross_spectral_features(train_path)
    target_feature, target_spectral_audit = fourier_cross_spectral_features(target_path)
    ...
    require(np.isfinite(probability).all(), "V134 probability invalid")
    return probability, {
        "train_n": len(ordered),
        ...
        "train_spectral": train_spectral_audit,
        "target_spectral": target_spectral_audit,
        "spectral_scaling": {
            "train_only": True,
            "clip": FEATURE_CLIP,
        ...
        "target_rows_used_for_robust_scale_spectral_scale_or_head": False,
        "target_labels_used": False,
        "learned_frequency_or_threshold": False,
        "random_fourier_or_rbf_kernel": False,
        "dmd_transition_operator_or_reservoir": False,
        "gaf_wavelet_signature_spd_dtw_or_rqa": False,
        "source_reliability_pairwise_auc_or_cantelli": False,
        "prediction": {
...
def choose_inner(
    ...
    challenger, model_audit = spectral_direction(inner_train, inner_valid)
    ...
    return {
        "selection_rule": "inner-past OOF only; exact V69 no-op or fixed .25/.50 causal Fourier cross-spectral blend; fixed 2*AUC+BA with BA/net eligibility",
        "baseline": baseline_metric,
        "selected": selected,
        "trials": trials,
        "outer_labels_used_for_selection": False,
        "frequency_terms_head_or_blend_micro_tuning": False,
    }, model_audit
...
def run_nested(
    ...
    diagnostic["v134_model"] = "V69_NOOP"
    ...
    for market, fold in ([(smoke_market, 2)] if smoke else fold_specs):
        ...
        outer_chronology = chronology(outer_train, outer_valid, f"V134 outer {market} fold {fold}")
        ...
        challenger, outer_model_audit = spectral_direction(outer_train, outer_valid)
        ...
        diagnostic.loc[positions, "v134_model"] = selected["name"]
        outer_diagnostics = {
        ...
        evidence["spectral_prob"] = challenger
        evidence["candidate_prob"] = candidate
        evidence["v134_model"] = selected["name"]
        ...
        print(
            f"[V134 FOURIER] market={market} fold={fold} selected={selected['name']} "
            f"inner_auc_delta={selected['auc_delta']:+.6f} outer_auc_delta={audits[-1]['outer_auc_delta']:+.6f}",
            flush=True,
        )
    evidence = pd.concat(evidence_parts, ignore_index=True)
    require(evidence.event_id.is_unique, "V134 evidence repeats an event")
    require(np.array_equal(champion.confidence_signal.to_numpy(float), diagnostic.confidence_signal.to_numpy(float)), "V134 changed confidence")
    require(np.array_equal(champion.high_conf.to_numpy(bool), diagnostic.high_conf.to_numpy(bool)), "V134 changed high_conf")
...
def paired_nested_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str, Any]:
    ...
    require(len(blocks) >= 12, "too few V134 nested-bootstrap blocks")
    ...
    require(len(values["auc_delta"]) >= int(0.90 * draws), "V134 bootstrap lost too many draws")
    ...
    return {
        ...
        "method": "paired market-fold-time block bootstrap over inner-locked V134 Fourier cross-spectral policy",
        "seed": SEED,
...
def evaluate(
    ...
    require(candidate_summary["research_gate"] == canonical_candidate, "V134 canonical mismatch")
    ...
    spectral_contract = all(
        audit[side]["causal_numeric_only"]
        and audit[side]["static_spec"]["feature_dimension"] == SPECTRAL_FEATURE_DIMENSION
        and audit[side]["static_spec"]["dc_removed_per_row_channel"]
        and not audit[side]["static_spec"]["learned_frequency_threshold_or_band"]
        and audit[side]["train_spectral"]["feature_dimension"] == SPECTRAL_FEATURE_DIMENSION
        and audit[side]["train_spectral"]["label_independent_exact_transform"]
        and audit[side]["train_spectral"]["parseval_max_abs_error"] <= 1e-10
        and audit[side]["target_spectral"]["parseval_max_abs_error"] <= 1e-10
        and audit[side]["target_spectral"]["unit_cross_max_abs"] <= 1.0000000001
        and audit[side]["spectral_scaling"]["train_only"]
        and not audit[side]["target_rows_used_for_robust_scale_spectral_scale_or_head"]
        and not audit[side]["target_labels_used"]
        and not audit[side]["learned_frequency_or_threshold"]
        and not audit[side]["random_fourier_or_rbf_kernel"]
        and not audit[side]["dmd_transition_operator_or_reservoir"]
        and not audit[side]["gaf_wavelet_signature_spd_dtw_or_rqa"]
        and not audit[side]["source_reliability_pairwise_auc_or_cantelli"]
    ...
    checks = {
        ...
        "multichannel_causal_fourier_cross_spectral_contract_verified": spectral_contract,
    }
    ...
    if not material_pass:
        require(selected_frame.equals(champion), "V134 fallback is not exact entire V69")
    return {
...
def material_gate(evaluation: dict[str, Any]) -> dict[str, Any]:
    ...
    require(gate["nested_bootstrap"]["contract_key"] == "selected.material_gate.nested_bootstrap", "V134 nested key mismatch")
    return gate
...
def write_outputs(
    ...
    require(bool(CONTROLLER_OUTPUT), "V134 full output requires MARKET_BIO_VERSION_OUTPUT authority")
    require(out.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V134 output is not controller-bound")
    ...
    reports = {
        "MODEL_COMPARISON.json": {
            "version": "V134", "hypothesis": HYPOTHESIS, "status": evaluation["status"],
            "models": {
                "V69_CHAMPION": evaluation["baseline_summary"],
                "V134_DIAGNOSTIC_FOURIER_CROSS_SPECTRAL": evaluation["candidate_summary"],
                "V134_FAIL_CLOSED_SELECTED": selected,
            },
        ...
        "V134_MULTICHANNEL_FOURIER_CROSS_SPECTRAL_REPORT.json": {
            "version": "V134", "hypothesis": HYPOTHESIS, "static_spec": STATIC_SPEC,
            "architectures": ARCHITECTURES, "nested_fold_audits": audits,
            "evaluation": compact, "authority_audit": authority,
        },
        "CANONICAL_RESEARCH_GATE_AUDIT.json": {
            "version": "V134", "status": "MATCH", "canonical_check_count": 14,
    ...
    atomic_csv(evidence, out / "V134_MULTICHANNEL_FOURIER_CROSS_SPECTRAL_OUTER_EVIDENCE.csv.gz")
    atomic_csv(evaluation["selected_frame"], out / "V134_FAIL_CLOSED_SELECTED_OOF.csv.gz")
...
def parse_args() -> argparse.Namespace:
    ...
    if CONTROLLER_OUTPUT:
        require(
            ...
            "explicit mode/output conflicts with controller-bound no-argument V134 full run",
        )
        args.full_run = True
    ...
    if args.full_run:
        require(bool(CONTROLLER_OUTPUT), "V134 direct full run forbidden without MARKET_BIO_VERSION_OUTPUT")
        require(args.output.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V134 controller full-run output mismatch")
    return args
...
def main() -> None:
    ...
    if args.audit_only:
        print(json.dumps(clean({
            ...
            "architecture": "exact horizon-axis rFFT log auto-power plus unit complex cross-spectrum phase coupling; fixed balanced ridge-logistic head",
            "causal_numeric_only": True, "target_row_labels_used": False,
            "calibration_threshold_band_or_post_outer_tuning": False,
            ...
            "controller_contract": {
                ...
                "research_staging_v134_compatible": True,
    ...
    with threadpool_limits(limits=SMOKE_THREADS if bounded else None):
        ...
        if args.support_probe:
            require(args.smoke_market == "KR", "V134 support probe is reserved for KR:2")
            audit = audits[0]
