"""V149 multichannel causal increment co-skewness tensor direction challenger.

The requested DTW class-medoid family is rejected because V87 already uses
rigid and Sakoe-Chiba class trajectory prototypes.  V149 instead extracts an
exact third-order increment representation from the observed eight-channel by
eight-horizon causal path.  Train-cut robust increment normalization precedes
per-row mean increments, all 120 unique symmetric third central co-moments,
and 28 time-oriented antisymmetric lag co-skewness terms.  The resulting fixed
156-dimensional representation is train-only robust-scaled and passed to one
fixed duplicate/class-balanced ridge-logit head.
...
DEFAULT_OUT = ROOT / "research" / "staging" / "V149" / "LOCAL_FORBIDDEN"
VERSION = 149
HYPOTHESIS = "MULTICHANNEL_CAUSAL_INCREMENT_COSKEWNESS_TENSOR_DIRECTION_V1"
...
HORIZON_N = 8
INCREMENT_N = 7
SYMMETRIC_TRIPLES = tuple(
    (left, middle, right)
    for left in range(CHANNEL_N)
    for middle in range(left, CHANNEL_N)
    for right in range(middle, CHANNEL_N)
)
CHANNEL_PAIRS = tuple(
    (left, right)
    for left in range(CHANNEL_N)
...
COSKEW_FEATURE_DIMENSION = CHANNEL_N + len(SYMMETRIC_TRIPLES) + len(CHANNEL_PAIRS)
INCREMENT_CLIP = 6.0
FEATURE_CLIP = 8.0
...
SMOKE_THREADS = 2
ARCHITECTURES = (
    {"name": "INCREMENT_COSKEWNESS_TENSOR_W0.25", "weight": 0.25},
    {"name": "INCREMENT_COSKEWNESS_TENSOR_W0.50", "weight": 0.50},
)
...
STATIC_SPEC = {
    ...
    "symmetric_third_central_comoment_n": len(SYMMETRIC_TRIPLES),
    "time_oriented_antisymmetric_lag_coskew_n": len(CHANNEL_PAIRS),
    "representation": [
        "train_normalized_mean_increment",
        "per_row_centered_unique_symmetric_third_central_comoment_tensor",
        "time_oriented_antisymmetric_lag_coskewness_by_channel_pair",
    ],
    "feature_dimension": COSKEW_FEATURE_DIMENSION,
    "feature_scaling": "train-only coordinate median/IQR-or-std and fixed clip",
    "head": "fixed duplicate/class-balanced ridge logistic",
    "learned_prototype_warping_threshold_frequency_graph_motif_or_embedding": False,
}
require(
    len(SYMMETRIC_TRIPLES) == 120
    and len(CHANNEL_PAIRS) == 28
    and COSKEW_FEATURE_DIMENSION == 156,
    "V149 fixed tensor dimension changed",
)
...
class TrainIncrementScale:
    def __init__(self) -> None:
        self.median = np.empty(CHANNEL_N, dtype=float)
        self.scale = np.ones(CHANNEL_N, dtype=float)

    def fit(self, path: np.ndarray) -> "TrainIncrementScale":
        require(path.ndim == 3 and path.shape[1:] == (HORIZON_N, CHANNEL_N), "V149 path shape invalid")
        flattened = np.diff(path, axis=1).reshape(-1, CHANNEL_N)
        ...
        require(np.isfinite(self.median).all() and np.isfinite(self.scale).all(), "V149 increment scale invalid")
        return self

    def transform(self, path: np.ndarray) -> np.ndarray:
        increment = np.diff(path, axis=1)
        normalized = np.clip((increment - self.median) / self.scale, -INCREMENT_CLIP, INCREMENT_CLIP)
        require(normalized.shape[1:] == (INCREMENT_N, CHANNEL_N), "V149 normalized increment shape invalid")
        require(np.isfinite(normalized).all(), "V149 normalized increment invalid")
        return normalized


def coskewness_features(normalized_increment: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    require(
        normalized_increment.ndim == 3
        and normalized_increment.shape[1:] == (INCREMENT_N, CHANNEL_N),
        "V149 increment tensor shape invalid",
    )
    mean_increment = np.mean(normalized_increment, axis=1)
    centered = normalized_increment - mean_increment[:, None, :]
    symmetric = np.stack([
        np.mean(centered[:, :, left] * centered[:, :, middle] * centered[:, :, right], axis=1)
        for left, middle, right in SYMMETRIC_TRIPLES
    ], axis=1)
    lagged = np.stack([
        np.mean(
            np.square(centered[:, :-1, left]) * centered[:, 1:, right]
            - np.square(centered[:, :-1, right]) * centered[:, 1:, left],
            axis=1,
        )
        for left, right in CHANNEL_PAIRS
    ], axis=1)
    feature = np.concatenate([mean_increment, symmetric, lagged], axis=1)
    require(feature.shape == (len(feature), COSKEW_FEATURE_DIMENSION), "V149 feature dimension changed")
    require(np.isfinite(feature).all(), "V149 co-skewness feature invalid")
    return feature, {
        "rows": len(feature),
        "feature_dimension": feature.shape[1],
        "mean_increment_q10_q50_q90": [float(np.quantile(mean_increment, q)) for q in (0.1, 0.5, 0.9)],
        "symmetric_coskew_q10_q50_q90": [float(np.quantile(symmetric, q)) for q in (0.1, 0.5, 0.9)],
        "oriented_lag_coskew_q10_q50_q90": [float(np.quantile(lagged, q)) for q in (0.1, 0.5, 0.9)],
        ...
        "third_central_comoment_tensor_verified": True,
        "time_oriented_antisymmetric_lag_branch_verified": True,
        "learned_knot_threshold_prototype_warping_frequency_graph_motif_or_embedding": False,
    }
...
class RobustFeatureScale:
    ...
    def fit(self, matrix: np.ndarray) -> "RobustFeatureScale":
        require(matrix.ndim == 2 and matrix.shape[1] == COSKEW_FEATURE_DIMENSION, "V149 scale shape changed")
        self.median = np.median(matrix, axis=0)
        q25, q75 = np.quantile(matrix, [0.25, 0.75], axis=0)
        robust = (q75 - q25) / 1.349
        standard = np.std(matrix, axis=0)
        self.scale = np.where(robust > 1e-8, robust, np.where(standard > 1e-8, standard, 1.0))
        require(np.isfinite(self.median).all() and np.isfinite(self.scale).all(), "V149 feature scale invalid")
        return self

    def transform(self, matrix: np.ndarray) -> np.ndarray:
        result = np.clip((matrix - self.median) / self.scale, -FEATURE_CLIP, FEATURE_CLIP)
        require(np.isfinite(result).all(), "V149 scaled feature invalid")
        return result


def coskewness_direction(train: pd.DataFrame, target_frame: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ordered = train.sort_values(["event_time_utc", "event_id"], kind="stable").reset_index(drop=True)
    require(ordered.market.nunique() == 1 and target_frame.market.nunique() == 1, "V149 expects one market per fit")
    ...
    increment_scale = TrainIncrementScale().fit(train_path)
    train_increment = increment_scale.transform(train_path)
    target_increment = increment_scale.transform(target_path)
    train_feature, train_tensor_audit = coskewness_features(train_increment)
    target_feature, target_tensor_audit = coskewness_features(target_increment)
    ...
    require(np.isfinite(probability).all(), "V149 probability invalid")
    return probability, {
        ...
        "train_geometry": train_tensor_audit, "target_geometry": target_tensor_audit,
        "increment_scaling": {
            "train_only": True, "clip": INCREMENT_CLIP,
            "median_sha256": array_sha256(increment_scale.median),
            "scale_sha256": array_sha256(increment_scale.scale),
        },
        "feature_scaling": {
        ...
        "dtw_medoid_or_class_prototype": False,
        "prediction": {
...
def configure_base() -> None:
    ...
    base.FRENET_FEATURE_DIMENSION = COSKEW_FEATURE_DIMENSION
    base.ARCHITECTURES = ARCHITECTURES
    base.SEED = SEED
    base.frenet_direction = coskewness_direction
...
def choose_inner(
    ...
    policy["selection_rule"] = "inner-past OOF only; exact V69 no-op or fixed .25/.50 increment co-skewness tensor blend; fixed 2*AUC+BA with BA/net/supported-source safety"
    policy["tensor_definition_head_blend_or_source_floor_micro_tuning"] = False
...
def run_nested(
    ...
    diagnostic = diagnostic.rename(columns={"v146_model": "v149_model"})
    evidence = evidence.rename(columns={"frenet_prob": "coskewness_prob", "v146_model": "v149_model"})
    for audit in audits:
        selected = audit["policy"]["selected"]
        print(
            f"[V149 INCREMENT COSKEWNESS] market={audit['market']} fold={audit['fold']} "
...
def paired_nested_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str, Any]:
    ...
    result["method"] = "paired market-fold-time block bootstrap over inner-locked V149 increment co-skewness tensor policy"
    result["seed"] = SEED
...
def evaluate(
    ...
    contract = bool(contract and all(
        audit[side]["train_geometry"]["third_central_comoment_tensor_verified"]
        and audit[side]["train_geometry"]["time_oriented_antisymmetric_lag_branch_verified"]
        and audit[side]["target_geometry"]["third_central_comoment_tensor_verified"]
        and audit[side]["target_geometry"]["time_oriented_antisymmetric_lag_branch_verified"]
        and audit[side]["increment_scaling"]["train_only"]
        and not audit[side]["dtw_medoid_or_class_prototype"]
        for audit in audits for side in ("inner_model_audit", "outer_model_audit")
    ))
    checks["multichannel_increment_coskewness_tensor_contract_verified"] = contract
    ...
    if not result["material_pass"]:
        ...
        require(result["selected_frame"].equals(champion), "V149 fallback not exact entire V69")
    return result
...
def material_gate(evaluation: dict[str, Any]) -> dict[str, Any]:
    ...
    require(gate["nested_bootstrap"]["contract_key"] == "selected.material_gate.nested_bootstrap", "V149 nested key mismatch")
    return gate


def enforce_output_conflict_fail_closed(out: Path) -> dict[str, Any]:
    require(bool(CONTROLLER_OUTPUT), "V149 output requires controller authority")
    require(out.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V149 output target is not controller-bound")
    require(out.resolve().parent == (ROOT / "research" / "staging" / "V149").resolve(), "V149 output is outside controller research/staging/V149")
    require(out.exists() and out.is_dir(), "V149 controller staging directory must already exist")
    ...
    require(required.issubset(existing), f"V149 controller staging prefiles missing: {sorted(required - existing)}")
    require(not (existing - allowed), f"V149 unexpected pre-existing output conflict: {sorted(existing - allowed)}")
    return {
...
def write_outputs(
    ...
    require(bool(CONTROLLER_OUTPUT) and out.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V149 output not controller-bound")
    conflict_audit = enforce_output_conflict_fail_closed(out)
    compact = {key: value for key, value in evaluation.items() if not key.endswith("_frame")}
    gate = material_gate(evaluation)
    selected = json.loads(json.dumps(clean(evaluation["selected_summary"])))
    selected["material_gate"] = gate
    require("bootstrap" in selected["robustness"], "V149 selected native bootstrap missing")
    reports = {
        "MODEL_COMPARISON.json": {
            "version": "V149", "hypothesis": HYPOTHESIS, "status": evaluation["status"],
            "models": {
                "V69_CHAMPION": evaluation["baseline_summary"],
                "V149_DIAGNOSTIC_INCREMENT_COSKEWNESS_TENSOR": evaluation["candidate_summary"],
                "V149_FAIL_CLOSED_SELECTED": selected,
            },
        ...
        "V149_INCREMENT_COSKEWNESS_TENSOR_REPORT.json": {
            "version": "V149", "hypothesis": HYPOTHESIS, "static_spec": STATIC_SPEC,
        ...
        "STANDARDIZED_NESTED_BOOTSTRAP_REPORT.json": {
            "version": "V149", "hypothesis": HYPOTHESIS,
        ...
        "CANONICAL_RESEARCH_GATE_AUDIT.json": {
            "version": "V149", "status": "MATCH", "canonical_check_count": 14,
    ...
    atomic_csv(evidence, out / "V149_INCREMENT_COSKEWNESS_OUTER_EVIDENCE.csv.gz")
    atomic_csv(evaluation["selected_frame"], out / "V149_FAIL_CLOSED_SELECTED_OOF.csv.gz")
    files = {}
...
def parse_args() -> argparse.Namespace:
    ...
    if CONTROLLER_OUTPUT:
        require(
            ...
            "explicit mode/output conflicts with controller-bound no-argument V149 full run",
        )
        args.full_run = True
    ...
    if args.full_run:
        require(bool(CONTROLLER_OUTPUT), "V149 direct full run forbidden without MARKET_BIO_VERSION_OUTPUT")
        require(args.output.resolve() == Path(CONTROLLER_OUTPUT).resolve(), "V149 controller full-run output mismatch")
    return args
...
def main() -> None:
    ...
    if args.audit_only:
        print(json.dumps(clean({
            ...
            "architecture": "fixed 156D multichannel increment third-central-co-moment and oriented lag-coskewness tensor plus balanced ridge logistic",
            "proposed_dtw_medoid_rejected_due_v87_direct_collision": True,
    ...
    with threadpool_limits(limits=SMOKE_THREADS if bounded else None):
        ...
        if args.support_probe:
            require(args.smoke_market == "KR", "V149 support probe is reserved for KR:2")
            audit = audits[0]
