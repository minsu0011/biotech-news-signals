"""V176 multichannel causal oriented level-crossing/excursion direction.

Each immutable V36 8-channel by 8-horizon causal path is scaled from its
strict-past train cut.  At five fixed standardized levels, every channel keeps
oriented upward/downward crossing counts and first locations, above/below dwell
runs, occupancy and positive/negative excess area.  Cross-channel synchronous
crossings and active-fraction trajectory changes add a fixed joint summary.
One equal-event-group/class-balanced ridge-logit head learns direction.  Levels
are pre-frozen; labels and target batches never define the representation.

This is not V137 CUSUM change-point geometry, V167 two-dimensional cubical
Euler curves, or V174 grayscale morphology.  V176 preserves temporal crossing
orientation and dwell/excess information within each channel and never builds
grid components, holes, openings or closings.  Inner OOF chooses exact V69 or
fixed .25/.50 blends, outer labels remain evaluation-only after the 35-minute
embargo/event purge, and any material failure restores the exact entire V69
frame with confidence/high_conf unchanged.
"""
...
base = scaffold.base
ROOT = scaffold.ROOT
DEFAULT_OUT = ROOT / "research" / "staging" / "V176" / "LOCAL_FORBIDDEN"
VERSION = 176
HYPOTHESIS = "MULTICHANNEL_CAUSAL_ORIENTED_LEVEL_CROSSING_EXCURSION_DIRECTION_V1"
...
CHANNEL_N = 8
HORIZON_N = 8
LEVELS = np.asarray([-1.0, -0.5, 0.0, 0.5, 1.0], dtype=float)
PER_CHANNEL_LEVEL_N = 9
JOINT_PER_LEVEL_N = 8
CROSSING_FEATURE_DIMENSION = CHANNEL_N * len(LEVELS) * PER_CHANNEL_LEVEL_N + len(LEVELS) * JOINT_PER_LEVEL_N
CHANNEL_CLIP = 7.0
FEATURE_CLIP = 8.0
...
SMOKE_THREADS = 2
ARCHITECTURES = (
    {"name": "ORIENTED_LEVEL_CROSSING_EXCURSION_W0.25", "weight": 0.25},
    {"name": "ORIENTED_LEVEL_CROSSING_EXCURSION_W0.50", "weight": 0.50},
)
...
STATIC_SPEC = {
    ...
    "fixed_standardized_levels": LEVELS.tolist(),
    "per_channel_level": ["above fraction", "up crossing rate", "down crossing rate", "first up location", "first down location", "longest above run", "longest below run", "positive excess area", "negative excess area"],
    "joint_per_level": ["mean/max/std synchronous up crossings", "mean/max/std synchronous down crossings", "active-fraction total variation", "active-fraction endpoint change"],
    "feature_dimension": CROSSING_FEATURE_DIMENSION,
    "head": "one equal-event-group/class-balanced C=0.5 ridge logistic",
    "learned_level_or_target_batch_fit": False,
}
require(CROSSING_FEATURE_DIMENSION == 400, "V176 crossing feature dimension changed")
...
def verify_authority() -> dict[str, Any]:
    ...
    audit["v176_access_contract"] = {"immutable_v36_dev_only": True, "atomic_output_v69_only": True, "failed_outputs_read": False, "dev_extension_read": False, "role_assignment_read": False, "research_seal_read": False, "final_reserve_read": False, "prior_version_output_read": False}
    return audit


class RobustFeatureScale:
    def __init__(self) -> None:
        self.median = np.zeros(CROSSING_FEATURE_DIMENSION)
        self.scale = np.ones(CROSSING_FEATURE_DIMENSION)

    def fit(self, matrix: np.ndarray) -> "RobustFeatureScale":
        require(matrix.shape[1] == CROSSING_FEATURE_DIMENSION, "V176 feature shape invalid")
    ...
    def transform(self, matrix: np.ndarray) -> np.ndarray:
        ...
        require(np.isfinite(result).all(), "V176 scaled feature invalid")
        return result
...
def longest_run(mask: np.ndarray) -> float:
    ...
    return float(longest / len(mask))


def first_crossing(mask: np.ndarray) -> float:
    positions = np.flatnonzero(mask)
    return float((positions[0] + 1) / (HORIZON_N - 1)) if len(positions) else -1.0


def channel_level_features(series: np.ndarray, level: float) -> np.ndarray:
    above = series >= level
    up = (~above[:-1]) & above[1:]
    down = above[:-1] & (~above[1:])
    return np.asarray([
        float(np.mean(above)), float(np.mean(up)), float(np.mean(down)),
        first_crossing(up), first_crossing(down),
        longest_run(above), longest_run(~above),
        float(np.mean(np.maximum(series - level, 0.0))),
        float(np.mean(np.maximum(level - series, 0.0))),
    ], dtype=float)


def crossing_features(path: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    require(path.ndim == 3 and path.shape[1:] == (HORIZON_N, CHANNEL_N), "V176 path shape invalid")
    result = np.empty((len(path), CROSSING_FEATURE_DIMENSION), dtype=float)
    upward_total = 0.0
    downward_total = 0.0
    for row in range(len(path)):
        ...
        for channel in range(CHANNEL_N):
            for level in LEVELS:
                current = channel_level_features(path[row, :, channel], float(level))
                upward_total += current[1]
                downward_total += current[2]
                parts.append(current)
        for level in LEVELS:
            active = path[row] >= level
            up_count = np.mean((~active[:-1]) & active[1:], axis=1)
            down_count = np.mean(active[:-1] & (~active[1:]), axis=1)
            active_fraction = np.mean(active, axis=1)
            parts.append(np.asarray([
                float(np.mean(up_count)), float(np.max(up_count)), float(np.std(up_count)),
                float(np.mean(down_count)), float(np.max(down_count)), float(np.std(down_count)),
                float(np.mean(np.abs(np.diff(active_fraction)))),
                float(active_fraction[-1] - active_fraction[0]),
            ]))
        result[row] = np.concatenate(parts)
    require(np.isfinite(result).all() and float(np.std(result)) > 1e-10, "V176 crossing feature degenerate")
    denominator = max(len(path) * CHANNEL_N * len(LEVELS), 1)
    return result, {
        "rows": len(result), "feature_dimension": result.shape[1], "feature_sha256": array_sha256(result),
        "fixed_standardized_levels": LEVELS.tolist(),
        "oriented_up_and_down_crossings": True,
        "first_crossing_locations": True,
        "above_below_dwell_runs": True,
        "positive_negative_excess_areas": True,
        "cross_channel_synchronous_crossings": True,
        "active_fraction_trajectory": True,
        "mean_up_crossing_rate": float(upward_total / denominator),
        "mean_down_crossing_rate": float(downward_total / denominator),
        "label_independent_exact_transform": True,
        "local_differential_geometry_not_global_path_integral": True,
        "learned_level_or_target_batch_fit": False,
        "v137_cusum_changepoint": False,
        "v167_grid_components_holes_or_euler": False,
        "v174_erosion_dilation_opening_closing": False,
        "v177_radon_tomography": False,
        "v178_cubic_bspline_functional_jet": False,
    }


def crossing_direction(train: pd.DataFrame, target_frame: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ordered = train.sort_values(["event_time_utc", "event_id"], kind="stable").reset_index(drop=True)
    require(ordered.market.nunique() == target_frame.market.nunique() == 1, "V176 expects one market per fit")
    ...
    train_feature, train_crossing = crossing_features(group_path)
    target_feature, target_crossing = crossing_features(target_path)
    ...
    require(np.isfinite(probability).all() and int(head.n_iter_[0]) < 1000, "V176 head invalid")
    return probability, {
        ...
        "train_geometry": train_crossing, "target_geometry": target_crossing,
...
def configure_base() -> None:
    base.VERSION = VERSION; base.HYPOTHESIS = HYPOTHESIS; base.STATIC_SPEC = STATIC_SPEC; base.FRENET_FEATURE_DIMENSION = CROSSING_FEATURE_DIMENSION; base.ARCHITECTURES = ARCHITECTURES; base.SEED = SEED; base.frenet_direction = crossing_direction
...
def choose_inner(inner_train: pd.DataFrame, inner_valid: pd.DataFrame, inner_champion: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any]]:
    challenger, model_audit = crossing_direction(inner_train, inner_valid); baseline = inner_champion.prob.to_numpy(float); confidence = inner_champion.confidence_signal.to_numpy(float); high = bool_series(inner_champion.high_conf).to_numpy(bool); baseline_metric = metric(inner_valid, baseline, confidence, high)
    ...
    model_eligible = bool(model_audit["causal_numeric_only"] and model_audit["grouping"]["equal_event_group_weighting"] and model_audit["channel_scaling"]["train_only"] and model_audit["feature_scaling"]["train_only"] and model_audit["train_geometry"]["oriented_up_and_down_crossings"] and model_audit["train_geometry"]["above_below_dwell_runs"] and model_audit["train_geometry"]["positive_negative_excess_areas"] and model_audit["train_geometry"]["cross_channel_synchronous_crossings"] and not model_audit["train_geometry"]["learned_level_or_target_batch_fit"] and not model_audit["target_rows_used_for_robust_scale_geometry_scale_or_head"] and not model_audit["target_labels_used"])
    ...
    return {"selection_rule":"inner-past OOF only; exact V69 noop or fixed .25/.50 oriented crossing/excursion blend with fixed safety","baseline":baseline_metric,"selected":selected,"trials":trials,"outer_labels_used_for_selection":False,"levels_summary_head_blend_or_source_floor_micro_tuning":False},model_audit
...
def run_nested(dev: pd.DataFrame, champion: pd.DataFrame, smoke: bool, smoke_market: str = "US") -> tuple[pd.DataFrame,pd.DataFrame,list[dict[str,Any]]]:
    ...
    diagnostic=diagnostic.rename(columns={"v146_model":"v176_model"});evidence=evidence.rename(columns={"frenet_prob":"crossing_prob","v146_model":"v176_model"})
    for audit in audits:
        selected=audit["policy"]["selected"];print(f"[V176 ORIENTED CROSSING] market={audit['market']} fold={audit['fold']} selected={selected['name']} inner_auc_delta={selected['auc_delta']:+.6f} outer_auc_delta={audit['outer_auc_delta']:+.6f}",flush=True)
    return diagnostic,evidence,audits


def paired_nested_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str,Any]:
    configure_base();result=_BASE_BOOTSTRAP(evidence,draws);result["method"]="paired market-fold-time block bootstrap over inner-locked V176 oriented crossing/excursion policy";result["seed"]=SEED;result["standardized"]=True;return result
...
def evaluate(champion: pd.DataFrame,diagnostic:pd.DataFrame,evidence:pd.DataFrame,audits:list[dict[str,Any]],draws:int,smoke:bool)->dict[str,Any]:
    configure_base();base.paired_nested_bootstrap=paired_nested_bootstrap;result=base.evaluate(champion,diagnostic,evidence,audits,draws,smoke);checks=result["material_checks"];checks.pop("multichannel_discrete_frenet_curvature_contract_verified");contract=bool(all(audit[side]["causal_numeric_only"] and audit[side]["static_spec"]["feature_dimension"]==CROSSING_FEATURE_DIMENSION and audit[side]["grouping"]["equal_event_group_weighting"] and audit[side]["train_geometry"]["oriented_up_and_down_crossings"] and audit[side]["train_geometry"]["first_crossing_locations"] and audit[side]["train_geometry"]["above_below_dwell_runs"] and audit[side]["train_geometry"]["positive_negative_excess_areas"] and audit[side]["train_geometry"]["cross_channel_synchronous_crossings"] and audit[side]["train_geometry"]["active_fraction_trajectory"] and not audit[side]["train_geometry"]["v137_cusum_changepoint"] and not audit[side]["train_geometry"]["v167_grid_components_holes_or_euler"] and not audit[side]["train_geometry"]["v174_erosion_dilation_opening_closing"] and audit[side]["channel_scaling"]["train_only"] and audit[side]["feature_scaling"]["train_only"] and not audit[side]["target_rows_used_for_robust_scale_geometry_scale_or_head"] and not audit[side]["target_labels_used"] for audit in audits for side in ("inner_model_audit","outer_model_audit")));checks["multichannel_causal_oriented_level_crossing_excursion_contract_verified"]=contract;result["material_pass"]=bool(not smoke and all(checks.values()))
    if not result["material_pass"]:result["selected_frame"]=champion.copy();result["selected_summary"]=result["baseline_summary"];result["fallback"]={"activated":True,"policy":"exact entire V69 DataFrame","exact_entire_frame_verified":bool(result["selected_frame"].equals(champion))};require(result["selected_frame"].equals(champion),"V176 fallback not exact V69")
    return result


def material_gate(evaluation:dict[str,Any])->dict[str,Any]:
    checks=evaluation["material_checks"];gate={"contract":HYPOTHESIS,"checks":checks,"passed":int(sum(bool(value) for value in checks.values())),"total":len(checks),"material_pass":bool(evaluation["material_pass"]),"nested_bootstrap":evaluation["nested_bootstrap"],"native_robustness_bootstrap":evaluation["candidate_native_robustness_bootstrap"],"current_hypothesis_not_inherited_from_v69":True,"fallback_policy":"candidate" if evaluation["material_pass"] else "exact entire V69 DataFrame"};require(gate["nested_bootstrap"]["contract_key"]=="selected.material_gate.nested_bootstrap","V176 nested key mismatch");return gate


def enforce_output_conflict_fail_closed(out:Path)->dict[str,Any]:
    require(bool(CONTROLLER_OUTPUT),"V176 output requires controller authority");require(out.resolve()==Path(CONTROLLER_OUTPUT).resolve(),"V176 output target mismatch");require(out.resolve().parent==(ROOT/"research"/"staging"/"V176").resolve(),"V176 output outside staging/V176");require(out.exists()and out.is_dir(),"V176 staging child must preexist");required={"VERSION_PLAN.json","BOTTLENECK_ANALYSIS.json","MATERIAL_CHANGE.json","RUN.log"};allowed=required|{"DATA_EPOCH_BINDING.json"};existing={path.name for path in out.iterdir()};require(required.issubset(existing),f"V176 prefiles missing: {sorted(required-existing)}");require(not(existing-allowed),f"V176 unexpected conflict: {sorted(existing-allowed)}");return {"controller_bound":True,"target_preexisting":True,"required_controller_prefiles":sorted(required),"observed_controller_prefiles":sorted(existing),"unexpected_prefiles":[],"conflict_policy":"allow exact controller prefiles only; fail closed before write"}
...
def write_outputs(out:Path,authority:dict[str,Any],evidence:pd.DataFrame,audits:list[dict[str,Any]],evaluation:dict[str,Any])->dict[str,Any]:
    require(bool(CONTROLLER_OUTPUT)and out.resolve()==Path(CONTROLLER_OUTPUT).resolve(),"V176 output not controller-bound");conflict=enforce_output_conflict_fail_closed(out);compact={key:value for key,value in evaluation.items()if not key.endswith("_frame")};gate=material_gate(evaluation);selected=json.loads(json.dumps(clean(evaluation["selected_summary"])));selected["material_gate"]=gate;require("bootstrap"in selected["robustness"],"V176 native bootstrap missing");reports={"MODEL_COMPARISON.json":{"version":"V176","hypothesis":HYPOTHESIS,"status":evaluation["status"],"models":{"V69_CHAMPION":evaluation["baseline_summary"],"V176_DIAGNOSTIC_ORIENTED_CROSSING":evaluation["candidate_summary"],"V176_FAIL_CLOSED_SELECTED":selected},"evaluation":compact,"authority_audit":authority,"nested_fold_audits":audits},"DEV_ROBUSTNESS_REPORT.json":{"version":"V176","hypothesis":HYPOTHESIS,"status":evaluation["status"],"opened_dev_only":True,"selected":selected,"candidate":evaluation["candidate_summary"],"material_gate":gate,"material_checks":evaluation["material_checks"],"nested_bootstrap":evaluation["nested_bootstrap"],"fallback_is_exact_entire_v69_frame":not evaluation["material_pass"],"seal_authorized":False},"SOURCE_TRANSFER_REPORT.json":{"version":"V176","hypothesis":HYPOTHESIS,"status":evaluation["status"],"selected_by_source_family":selected["metrics"]["by_source_family"],"candidate_by_source_family":evaluation["candidate_summary"]["metrics"]["by_source_family"],"selected_by_market":selected["metrics"]["by_market"],"candidate_by_market":evaluation["candidate_summary"]["metrics"]["by_market"],"material_gate":gate,"nested_bootstrap":evaluation["nested_bootstrap"],"fallback_is_exact_entire_v69_frame":not evaluation["material_pass"],"seal_authorized":False},"V176_ORIENTED_LEVEL_CROSSING_EXCURSION_REPORT.json":{"version":"V176","hypothesis":HYPOTHESIS,"static_spec":STATIC_SPEC,"architectures":ARCHITECTURES,"nested_fold_audits":audits,"evaluation":compact,"authority_audit":authority},"STANDARDIZED_NESTED_BOOTSTRAP_REPORT.json":{"version":"V176","hypothesis":HYPOTHESIS,"contract_key":"selected.material_gate.nested_bootstrap","bootstrap":evaluation["nested_bootstrap"]},"NATIVE_ROBUSTNESS_BOOTSTRAP_REPORT.json":{"version":"V176","hypothesis":HYPOTHESIS,"controller_native_robustness_bootstrap":evaluation["candidate_native_robustness_bootstrap"]},"CANONICAL_RESEARCH_GATE_AUDIT.json":{"version":"V176","status":"MATCH","canonical_check_count":14,"reported":selected["research_gate"],"controller_recomputed":controller.canonical_research_gate(selected),"material_gate":gate},"RUN_STATUS.json":{"version":VERSION,"hypothesis":HYPOTHESIS,"status":evaluation["status"],"material_pass":evaluation["material_pass"],"champion_changed":evaluation["material_pass"],"seal_state":"UNOPENED","seal_authorized":False,"output_conflict_audit":conflict,"completed_at":pd.Timestamp.now(tz="UTC").isoformat()}}
    for name,payload in reports.items():atomic_json(payload,out/name)
    atomic_csv(evidence,out/"V176_ORIENTED_CROSSING_OUTER_EVIDENCE.csv.gz");atomic_csv(evaluation["selected_frame"],out/"V176_FAIL_CLOSED_SELECTED_OOF.csv.gz");files={path.name:{"sha256":sha256(path),"bytes":path.stat().st_size}for path in sorted(out.iterdir())if path.is_file()and path.name!="ARTIFACT_MANIFEST.json"};atomic_json({"version":VERSION,"hypothesis":HYPOTHESIS,"authority_experiment_id":EXPECTED_V69_EXPERIMENT_ID,"files":files},out/"ARTIFACT_MANIFEST.json");return {"output":str(out),"artifact_count":len(files)+1,"manifest_sha256":sha256(out/"ARTIFACT_MANIFEST.json")}
...
def parse_args()->argparse.Namespace:
    ...
    if CONTROLLER_OUTPUT:require(not args.audit_only and not args.smoke_test and not args.support_probe and not args.full_run and args.output is None,"explicit mode/output conflicts with controller-bound no-argument V176 full run");args.full_run=True;args.output=Path(CONTROLLER_OUTPUT)
    elif args.output is None:args.output=DEFAULT_OUT
    if args.full_run:require(bool(CONTROLLER_OUTPUT),"V176 direct full run forbidden without MARKET_BIO_VERSION_OUTPUT");require(args.output.resolve()==Path(CONTROLLER_OUTPUT).resolve(),"V176 controller full-run output mismatch")
    return args
