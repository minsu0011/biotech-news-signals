"""V92 exact local-Linf adversarial-margin direction challenger.

The material hypothesis is local state robustness.  A static causal-numeric
logit is not trained on average observed-row loss: for every row it minimizes
the exact worst-case logistic loss over an L-infinity ball in training-cut
robust-scaled feature space.  Linear duality gives the closed-form adverse
margin epsilon*||w||_1, avoiding approximate attacks and target-row access.
...
and one two-thread single-fold smoke only; no implicit full run occurs.
"""
...
DEFAULT_OUT = ROOT / "research" / "staging" / "V92_EXACT_LOCAL_LINF_ADVERSARIAL_MARGIN_V1"

VERSION = 92
HYPOTHESIS = "EXACT_LOCAL_LINF_ADVERSARIAL_MARGIN_DIRECTION_V1"
...
# Strictly causal pre-decision numeric market state.  Entry-bar, source,
# category, issuer, ticker, text, keyword, historical response, and post-event
# variables are excluded.  The adversary perturbs only these scaled scalars.
FEATURES = (
...
def load_authorized() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    require(not missing, f"V92 causal feature missing: {missing}")
...
def inner_partition(
    ...
    audit = chronology(inner_train, inner_valid, "V92 inner")
...
class RobustState:
    ...
    def fit(self, frame: pd.DataFrame) -> "RobustState":
        ...
        require(np.isfinite(self.median).all() and np.isfinite(self.scale).all(), "robust state fit failed")
        return self

    def transform(self, frame: pd.DataFrame) -> np.ndarray:
        require(self.median is not None and self.scale is not None, "robust state not fitted")
        values = frame.loc[:, FEATURES].to_numpy(float)
        filled = np.where(np.isfinite(values), values, self.median)
        output = np.clip((filled - self.median) / self.scale, -7.0, 7.0).astype(np.float32)
        require(np.isfinite(output).all(), "non-finite robust state")
        return output


def exact_robust_loss(
    logits: torch.Tensor, target: torch.Tensor, weight_vector: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    signed_target = 2.0 * target - 1.0
    clean_margin = signed_target * logits
    dual_penalty = L_INF_RADIUS * torch.sum(torch.abs(weight_vector))
    robust_margin = clean_margin - dual_penalty
    row_loss = nn.functional.softplus(-robust_margin)
    return row_loss, clean_margin, robust_margin


def fit_adversarial_margin(
    train: pd.DataFrame, target: pd.DataFrame, seed: int, smoke: bool,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    processor = RobustState().fit(train)
    train_x = processor.transform(train)
    target_x = processor.transform(target)
    ...
    sample_weight = torch.from_numpy(train.cluster_weight.to_numpy(np.float32)).to(device)
    ...
    for epoch in range(epochs):
        ...
        row_loss, clean_margin, robust_margin = exact_robust_loss(
            logits, y_tensor, model.weight.squeeze(0)
        )
        objective = torch.sum(row_loss * sample_weight) / torch.sum(sample_weight)
        objective.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
        optimizer.step()
        if epoch in {0, epochs // 2, epochs - 1}:
            with torch.no_grad():
                clean_bce = nn.functional.softplus(-clean_margin)
                dual = L_INF_RADIUS * torch.sum(torch.abs(model.weight))
            trace.append({
                "epoch": epoch + 1,
                "exact_worst_case_bce": float(objective.detach()),
                "clean_weighted_bce": float(torch.sum(clean_bce * sample_weight) / torch.sum(sample_weight)),
                "dual_margin_penalty": float(dual),
                "certified_training_fraction": float(torch.mean((robust_margin > 0.0).float())),
            })
    with torch.no_grad():
        final_train_logits = model(x_tensor).squeeze(1)
        final_row_loss, final_clean_margin, final_robust_margin = exact_robust_loss(
            final_train_logits, y_tensor, model.weight.squeeze(0)
        )
        target_logits = model(torch.from_numpy(target_x).to(device)).squeeze(1)
        dual_penalty = L_INF_RADIUS * torch.sum(torch.abs(model.weight.squeeze(0)))
        target_certified_margin = torch.clamp(torch.abs(target_logits) - dual_penalty, min=0.0)
        target_certified_logit = torch.sign(target_logits) * target_certified_margin
        probability = torch.sigmoid(target_logits).cpu().numpy()
        certified_probability = torch.sigmoid(target_certified_logit).cpu().numpy()
        target_logit_values = target_logits.cpu().numpy()
        target_certified_values = target_certified_margin.cpu().numpy()
        coefficient = model.weight.squeeze(0).cpu().numpy()
    final_clean_loss = nn.functional.softplus(-final_clean_margin)
    clean_weighted_bce = float(torch.sum(final_clean_loss * sample_weight) / torch.sum(sample_weight))
    robust_weighted_bce = float(torch.sum(final_row_loss * sample_weight) / torch.sum(sample_weight))
    require(robust_weighted_bce >= clean_weighted_bce - 1e-8, "worst-case loss below clean loss")
    require(np.isfinite(probability).all(), "non-finite adversarial-margin probability")
    predictions = {
        "probability": np.clip(np.asarray(probability, float), 1e-6, 1.0 - 1e-6),
        "certified_probability": np.clip(np.asarray(certified_probability, float), 1e-6, 1.0 - 1e-6),
        "logit": np.asarray(target_logit_values, float),
        "certified_margin": np.asarray(target_certified_values, float),
    }
    audit = {
        "architecture": "static linear logit with exact local L-infinity adversarial-margin objective",
        "duality": "min over ||delta||_inf<=epsilon of y*(w dot (x+delta)+b) equals clean signed margin minus epsilon*||w||_1",
        "approximate_attack_used": False,
        "l_inf_radius": L_INF_RADIUS,
        "dual_norm": "L1 coefficient norm",
        "dual_margin_penalty": float(L_INF_RADIUS * np.sum(np.abs(coefficient))),
        "coefficient_l1": float(np.sum(np.abs(coefficient))),
        "coefficient_l2": float(np.linalg.norm(coefficient)),
        "clean_weighted_bce": clean_weighted_bce,
        "exact_worst_case_weighted_bce": robust_weighted_bce,
        "certified_training_fraction": float(torch.mean((final_robust_margin > 0.0).float())),
        "target_certified_fraction": float(np.mean(target_certified_values > 0.0)),
        "target_clean_logit_quantiles": {
            str(q): float(np.quantile(target_logit_values, q)) for q in (0.1, 0.5, 0.9)
        },
        "target_certified_margin_quantiles": {
            str(q): float(np.quantile(target_certified_values, q)) for q in (0.1, 0.5, 0.9)
        },
        ...
        "environment_or_sample_reweighting_used": False,
        "kernel_density_tree_graph_trajectory_filter_anchor_used": False,
        "device": "CPU", "gpu_used": False,
        "cpu_reason": "bounded convex-like 37-feature exact dual-margin optimization",
    }
...
def choose_inner(
    ...
    predictions, model_audit = fit_adversarial_margin(train, valid, SEED + fold * 10, smoke)
    ...
    for weight in BLEND_WEIGHTS:
        ...
        trials.append({
            "name": f"EXACT_LINF_MARGIN_W{weight:.2f}", "weight": float(weight),
    ...
    return {
        ...
        "adversarial_margin_model_audit": model_audit,
...
def run_nested(
    ...
    diagnostic["v92_model"] = "V69_NOOP"
    diagnostic["v92_blend_weight"] = 0.0
    evidence_parts = []
    audits = []
    for fold in (2, 3, 4):
        ...
        outer_audit = chronology(outer_train, outer_valid, f"V92 outer fold {fold}")
        ...
        predictions, outer_model_audit = fit_adversarial_margin(
        ...
        diagnostic.loc[positions, "v92_model"] = selected["name"]
        diagnostic.loc[positions, "v92_blend_weight"] = weight
        ...
        evidence["adversarial_margin_prob"] = predictions["probability"]
        evidence["certified_probability"] = predictions["certified_probability"]
        evidence["model_logit"] = predictions["logit"]
        evidence["certified_margin"] = predictions["certified_margin"]
        evidence["candidate_prob"] = candidate
        evidence["v92_model"] = selected["name"]
        evidence["v92_blend_weight"] = weight
        evidence_parts.append(evidence)
        audits.append({
            "fold": fold, "inner_chronology": inner_audit, "outer_chronology": outer_audit,
            "policy": policy, "outer_adversarial_margin_model_audit": outer_model_audit,
            ...
            "outer_certified_fraction": float(np.mean(predictions["certified_margin"] > 0.0)),
            "policy_locked_before_outer_evaluation": True,
            "outer_labels_used_for_selection": False,
        })
        print(
            f"[V92 EXACT-LINF] fold={fold} selected={selected['name']} "
            f"inner_auc_delta={selected['auc_delta']:+.6f} outer_auc_delta={audits[-1]['outer_auc_delta']:+.6f} "
            f"certified_fraction={audits[-1]['outer_certified_fraction']:.4f}",
            flush=True,
        )
        if smoke:
            break
...
def paired_bootstrap(evidence: pd.DataFrame, draws: int) -> dict[str, Any]:
    ...
    return {
        "method": "paired fold/date block bootstrap over locked V92 exact-Linf policies",
...
def evaluate(
    ...
    model_audits = [audit["policy"]["adversarial_margin_model_audit"] for audit in audits] + [
        audit["outer_adversarial_margin_model_audit"] for audit in audits
    ]
    checks = {
        ...
        "exact_local_adversarial_objective_all_fits": all(
            audit["l_inf_radius"] == L_INF_RADIUS
            and audit["dual_norm"] == "L1 coefficient norm"
            and not audit["approximate_attack_used"]
            and audit["exact_worst_case_weighted_bce"] >= audit["clean_weighted_bce"] - 1e-8
        ...
        "no_environment_sample_or_anchor_surrogate": all(
            not audit["environment_or_sample_reweighting_used"]
            and not audit["kernel_density_tree_graph_trajectory_filter_anchor_used"] for audit in model_audits
        ),
        "certified_margin_nonconstant": bool(
            np.isfinite(evidence.certified_margin.to_numpy(float)).all()
            and float(evidence.certified_margin.std()) > 1e-10
        ),
    ...
    return {
        ...
        "certificate_diagnostics": {
            "model_logit_quantiles": {
                str(q): float(np.quantile(evidence.model_logit, q)) for q in (0.1, 0.5, 0.9)
            },
            "certified_margin_quantiles": {
                str(q): float(np.quantile(evidence.certified_margin, q)) for q in (0.1, 0.5, 0.9)
            },
            "certified_fraction": float(np.mean(evidence.certified_margin > 0.0)),
        },
...
def write_outputs(
    ...
    require(not out.name.startswith("output_V92"), "runner must not write output_V92 directly")
    ...
    reports = {
        "MODEL_COMPARISON.json": {
            "version": "V92", "hypothesis": HYPOTHESIS, "status": evaluation["status"],
            "models": {
                "V69_CHAMPION": evaluation["baseline_summary"],
                "V92_EXACT_LINF_DIAGNOSTIC": evaluation["candidate_summary"],
                "V92_FAIL_CLOSED_SELECTED": evaluation["selected_summary"],
            },
        ...
        "V92_EXACT_LOCAL_LINF_MARGIN_REPORT.json": {
            "version": "V92", "hypothesis": HYPOTHESIS,
            "features": FEATURES, "l_inf_radius": L_INF_RADIUS,
            "blend_weights": BLEND_WEIGHTS, "authority_audit": authority,
            "nested_fold_audits": audits, "evaluation": compact,
        },
        "V92_IMMUTABILITY_AUDIT.json": {
    ...
    atomic_csv(evidence, out / "V92_EXACT_LINF_OUTER_OOF.csv.gz")
    atomic_csv(evaluation["selected_frame"], out / "V92_FAIL_CLOSED_SELECTED_OOF.csv.gz")
    return {
        "runner_files_written": sorted([
            *reports, "V92_EXACT_LINF_OUTER_OOF.csv.gz", "V92_FAIL_CLOSED_SELECTED_OOF.csv.gz",
        ])
    }
...
def main() -> None:
    ...
    if args.audit_only:
        print(json.dumps(clean({
            ...
            "model_contract": {
                "objective": "exact worst-case logistic loss inside a standardized-state L-infinity ball",
                "l_inf_radius": L_INF_RADIUS,
                "closed_form_dual": "epsilon times L1 coefficient norm",
                "model_inputs": "37 causal pre-event numeric scalars only",
                "device": "CPU", "gpu_used": False,
            },
            "prior_art_audit": {
                "versions": "V37-V91 root runners plus fixed V91 declaration",
                "local_feature_adversarial_or_certified_margin_found": False,
                "disjoint_from_v84_v90": "no kernel posterior, density, dependency tree, or generative likelihood",
                "disjoint_from_v81_v85": "no environment adversary and no sample entropy reweighting",
                "disjoint_from_v91": "no chronological anchors, residual projection, or invariant ridge penalty",
            },
    ...
    summary = {
        ...
        "adversarial_margin_fit_count": 2 * len(audits),
        ...
        "certificate_diagnostics": evaluation["certificate_diagnostics"],
